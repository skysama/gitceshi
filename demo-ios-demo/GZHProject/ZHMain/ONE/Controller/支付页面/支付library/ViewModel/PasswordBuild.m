...
- (void)createDefaultData {
    ...
    self.inputPwd.completeHandle = ^(NSString *inputPwd ,NSString *paystyle){
    ...
    self.reInitPwd.completeHandle = ^(NSString *inputPwd,NSString *paystyle ){
        NSLog(@"重新设置输入：%@",inputPwd);
        weakSelf.reInitPwdText = inputPwd;
        [weakSelf changeFinalPwdFrame];
    };
    
    self.finalPwd.completeHandle = ^(NSString *inputPwd,NSString *paystyle){
        NSLog(@"再次输入：%@",inputPwd);
        //如果输入不同，清除
